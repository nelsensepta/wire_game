  ...
  stop: function () {
    ...
    fetch(path + "/insert_score.php?id=" + this.id, {
      ...
      .then((res) => {
        ...
        resultName.innerHTML = res.data.name;
      });
  },
};
...
document.onreadystatechange = () => {
  if (document.readyState === "complete") {
    ...
    leftNavigation.addEventListener("mousedown", function () {
      if (!jumping) {
        jumping = true;
        Player.jumping("left", () => {
          jumping = false;
    ...
    rightNavigation.addEventListener("mousedown", function () {
      if (!jumping) {
        jumping = true;
